...
function buildPromptForMUP(highlightedText, multiUserPrompt) { 
...
export async function requestResponseForMUP(model, highlightedText, multiUserPrompt) {
  
  // If Ollama is selected
  if(model == "Ollama"){
    try {
      return await getOllamaResponse(buildPromptForMUP(highlightedText, multiUserPrompt));  
    } catch (error) {
  ...
  if(model == "OpenAI"){
    try {
      return await getOpenAIResponse(buildPromptForMUP(highlightedText, multiUserPrompt));  
    } catch (error) {
      console.error('Error:', error.message || error);
      return error;
    }
